...
class DataBase:
    query = {
        ...
        'max_company_id': text('selest MAX(id) from company'),
    ...
    """Инициилизируем класс и двигатель БД"""
    ...
    def get_employer_id(self, company_id: int):
        try:
            with self.db.connect() as connection:
                result = connection.execute(self.query['maxID_SELECT'], parameters=dict(c_id=company_id))[
                    0]
                return result
        except Exception as _ex:
            print("[INFO] Error - can't work with SQL", _ex)
